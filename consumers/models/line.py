...
class Line:
    ...
    def _handle_arrival(self, message):
        ...
        #print(f"{value.get('direction')}, {value.get('train_id')}, {value.get('train_status')}")
    ...
    def process_message(self, message):
        ...
        if topic == 'org.chicago.cta.stations.table.v1': # the topic for the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif re.search("^org.chicago.cta.station.arrivals.", topic): # the arrival topic
            #print(f"_handle_arrival for {topic}")
            self._handle_arrival(message)
        elif topic == 'TURNSTILE_SUMMARY': # the KSQL Turnstile Summary Topic
            #print("topic: TURNSTILE_SUMMARY")
            json_data = json.loads(message.value())
            station_id = json.loads(message.key())
            #print(f"key {station_id}")
